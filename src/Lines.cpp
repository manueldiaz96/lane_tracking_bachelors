...
class Line_Detector
{
  ...
  public:
    ...
    { 
      ...
      std::vector<int> LastLeftLine;
      std::vector<int> LastRightLine;

      nh_.getParam("leftLinek1", LastLeftLine);
      nh_.getParam("rightLinek1", LastRightLine);
      ...
      std::vector<int> yar;
      nh_.getParam("horizonLine", yar);
      yar[2] = yar[1];
      yar[1] = yar[0];
      yar[0] = m1*x_h + b1_int;
      ya = (yar[2]+yar[1]+yar[0])/3;
      nh_.setParam("horizonLine", yar);
      ...
      std::vector<int> leftLine_param, leftLinek1_param, leftLinek2_param, leftLinek3_param;
      std::vector<int> rightLine_param, rightLinek1_param, rightLinek2_param, rightLinek3_param;

      for (int i = 0; i < 5; ++i)
      {
        leftLine_param.push_back(LeftLine[i]);
        rightLine_param.push_back(RightLine[i]);
      }
  
      nh_.getParam("leftLinek3", leftLinek3_param);
      nh_.getParam("leftLinek2", leftLinek2_param);
      nh_.getParam("leftLinek1", leftLinek1_param);

      /*nh_.getParam("rightLinek3", rightLinek3_param);
      nh_.getParam("rightLinek2", rightLinek2_param);
      nh_.getParam("rightLinek1", rightLinek1_param);*/

      for (int i = 0; i < 5; ++i)
      {
        leftLinek3_param[i] = leftLinek2_param[i];
        leftLinek2_param[i] = leftLinek1_param[i];
        leftLinek1_param[i] = LeftLine[i];
        LeftLine[i] = (LeftLine[i] + leftLinek1_param[i] + leftLinek2_param[i] + leftLinek3_param[i])/4;

        /*rightLinek3_param[i] = rightLinek2_param[i];
        rightLinek2_param[i] = rightLinek1_param[i];
        rightLinek1_param[i] = RightLine[i];
        RightLine[i] = (RightLine[i] + rightLinek1_param[i] + rightLinek2_param[i] + rightLinek3_param[i])/4;*/
      }

      nh_.setParam("leftLinek3", leftLinek3_param);
      nh_.setParam("leftLinek2", leftLinek2_param);
      nh_.setParam("leftLinek1", leftLinek1_param);

     /* nh_.setParam("rightLinek3", rightLinek3_param);
      nh_.setParam("rightLinek2", rightLinek2_param);*/
      nh_.setParam("rightLinek1", rightLine_param);
